...
for p in densities:
    ...
    while acyclic < number_of_experiments:
        ...
        par_sep = mrx.SD_CPDAGs(CPDAG_G,CPDAG_H,type='pparent',normalized = True, MB_enhanced = False)


        parent_sep.append(par_sep)

        ZL_sep.append(mrx.SD_CPDAGs(CPDAG_G,CPDAG_H,type='ZL',normalized = True, MB_enhanced = False))

        #print(mrx.SD_CPDAGs(CPDAG_G,CPDAG_H,type='ZL',normalized = True, MB_enhanced = False))

        MB_enhanced_parent_sep.append(mrx.SD_CPDAGs(CPDAG_G, CPDAG_H, type='pparent', normalized=True, MB_enhanced=True))

        par_AID = mrx.parent_AID_CPDAGs(CPDAG_G,CPDAG_H)

        parent_AID.append( par_AID)

        SHD_case = mrx.SHD_CPDAGs(CPDAG_G,CPDAG_H)
...
ax1.plot(densities,parent_SD_list,color = '#CC79A7',label = 'ppSD')
...
ax1.plot(densities,MB_enhanced_SD.values(),label = 'MB-ppSD')
...
filename = 'Plots/Differences/'+ 'experiments_CPDAGs_Figure_main_text' + '.png'
