...
for p in densities:
    ...
    while acyclic < number_of_experiments:
        ...
        par_sep = mrx.SD_DAGs(H,G,type='parent',normalized = True, MB_enhanced = False)

        parent_sep.append(par_sep)

        ZL_sep.append(mrx.SD_DAGs(H,G,type='ZL',normalized = True, MB_enhanced = False))

        MB_enhanced_parent_sep.append(mrx.SD_DAGs(H, G, type='parent', normalized=True, MB_enhanced=True))

        par_AID = mrx.parent_AID_DAGs(H,G)

        parent_AID.append(par_AID)

        SHD_case = mrx.SHD_DAGs(H,G)
        print(SHD_case)
...
filename = 'Plots/Differences/'+ 'experiments_DAGs_Figure_main_text_roles_changed' + '.png'
