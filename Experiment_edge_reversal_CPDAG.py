...
while all_acyclic < number_of_experiments:
    ...
    graphs = []
    base = fcs.generate_ER_random_DAG(N, p, random_state=random_state)
    graphs.append(base)
    CPDAGs = []

    CPDAGs.append(base.get_CPDAG())
    ...
    for r in range(number_of_reversals):
        ...
        while t < attempts:
            ...
            if nx.is_directed_acyclic_graph(try_graph.to_nx()):
                ...
                CPDAGs.append(try_graph.get_CPDAG())
                break
    ...
    for r in range(1,number_of_reversals+1):
        parent_sep[all_acyclic].append(mrx.SD_CPDAGs(CPDAGs[0],CPDAGs[r],type='pparent',normalized = True, MB_enhanced = False))
        ZL_sep[all_acyclic].append(mrx.SD_CPDAGs(CPDAGs[0],CPDAGs[r], type='ZL', normalized=True, MB_enhanced=False))

        MB_enhanced_parent_sep[all_acyclic].append(mrx.SD_CPDAGs(CPDAGs[0],CPDAGs[r], type='pparent', normalized=True, MB_enhanced=True))

        parent_AID[all_acyclic].append(mrx.parent_AID_CPDAGs(CPDAGs[0],CPDAGs[r]))

        SHD_list[all_acyclic].append(mrx.SHD_CPDAGs(CPDAGs[0],CPDAGs[r]))
...
filename = 'Plots/Differences/'+ 'experiments_several_edge_reversals_CPDAGs' + '.png'
